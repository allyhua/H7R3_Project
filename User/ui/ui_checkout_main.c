...
static void checkout_ui_update_preview(void)
{
    ...
    zoom_x = box_w * 256U / g_ui.workflow->frame_width;
    zoom_y = box_h * 256U / g_ui.workflow->frame_height;
...
static void checkout_ui_show_notice(camera_workflow_state_t *state)
{
    ...
    {
        checkout_ui_show_error("提示", (notice_text[0] != '\0') ? notice_text : "请重�?);
    }
}
...
static void checkout_ui_build_layout(void)
{
    ...
    lv_label_set_text(g_ui.total_unit_label, "�?);
    ...
    checkout_ui_apply_font(button_label);
    lv_obj_set_style_text_color(button_label, lv_color_hex(0xFFFFFF), 0);
    lv_label_set_text(button_label, "删除选中�?);
    lv_obj_center(button_label);
    ...
    checkout_ui_apply_font(button_label);
    lv_obj_set_style_text_color(button_label, lv_color_hex(0xFFFFFF), 0);
    lv_label_set_text(button_label, "清空购物�?);
    lv_obj_center(button_label);
...
void checkout_ui_init(camera_workflow_state_t *state)
{
    ...
    g_ui.preview_dsc.data_size = state->frame_width * state->frame_height * 2U;
...
void checkout_ui_set_status(const char *status_text)
{
    ...
    lv_label_set_text(g_ui.status_label, (status_text != NULL) ? status_text : UI_TEXT_PLACEHOLDER);
}
...
void checkout_ui_set_recognition_result(const cart_add_result_t *result)
{
    ...
    else
    {
        snprintf(text,
                 ...
                 (product != NULL) ? product->display_unit_text : "�?);
    }
...
void checkout_ui_refresh_cart(const cart_context_t *cart, int32_t selected_index)
{
    ...
    for (idx = 0U; idx < PRODUCT_ID_COUNT; ++idx)
    {
        ...
        lv_label_set_text(top_name,
                          ((item->product_name[0] != '\0') ? item->product_name
                                                           : ((product != NULL) ? product->product_name : UI_TEXT_PLACEHOLDER)));
        ...
        snprintf(price_text,
                 ...
                 (product != NULL) ? product->price_unit_text
                                   : ((item->pricing_mode == PRODUCT_PRICING_BY_WEIGHT) ? "�?千克" : "�?�?));
    ...
    if (valid_count == 0U)
    {
        ...
        lv_label_set_text(empty_label, "购物车为�?);
...
static void checkout_ui_finish_event_cb(lv_event_t *e)
{
    ...
    {
        checkout_ui_show_error("结账", "购物车为�?);
        return;
    }
...
static void ui_checkout_refresh_all(camera_workflow_state_t *state)
{
    ...
    {
        lv_label_set_text(g_ui.result_name_label, "商品：识别失�?);
        lv_label_set_text(g_ui.result_price_label, "单价�?-");
        lv_label_set_text(g_ui.result_amount_label, "重量/数量�?-");
